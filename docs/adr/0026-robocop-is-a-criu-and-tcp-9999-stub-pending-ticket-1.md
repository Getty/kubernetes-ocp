...
> which is **not wired up**. The eventual replacement is ticket k1 (re-add
...
Ticket k1 (`Adopt Net::Async::Kubernetes as the watch driver; implement
...
- **The stub is what runs, not what we want.** Anything that depends on
  ...
  by ticket k1, not by what is checked in.
...
- **Pull the stub and ship `OCP::Robocop::Controller` instead** — would
  require closing ticket k1 first (typed client, watch loop, port-forward
...
- **Rename and forget** — leaving the CRIU path undocumented means the next
  ...
  k1, and may either delete the listener (silently breaking key injection
...
- **Document the design AND the current state in two ADRs** — the design is
  ticket k1's payload and does not yet exist as a written decision. Recording
  it here would be writing a forward reference. When ticket k1 lands, the new
...
- Anyone who needs robocop actually reconciling `OCPNode` from inside the
  cluster has to land ticket k1 first. The dependency is recorded, not
  hidden.
- The CRIU binary dependency is not removed because the stub uses it; that
  change rides in with ticket k1, when the stub goes away. Until then, the
...
- This ADR is the source of truth for what is shipped now. When ticket k1
...
- The CRIU/TCP-9999 design will not be modified in place. If ticket k1 is
